...
成熟度：`Alpha`。源码：`crates/slint-ui/ui/controls/tool-button.slint`。公开名称：`ToolButton`。
...
Gallery“通用 / ToolButton”页展示 checked ToolButton；自动测试验证激活次数及 checked 单次切换。跨子项方向键 roving focus 不在 ToolButton 内实现。
