...
当前仓库已完成全部 P0、提前完成的 `ScrollArea`、里程碑 D 核心桌面批次，以及里程碑 E 的全部 20 个 P2 核心组件。已实现组件的 API 和限制见 [`component-specs/`](component-specs/README.md)；剩余 P1、质量补齐、平台验证、产品接入和发布任务统一在 [`../TODO.md`](../TODO.md) 维护。本文件只定义目标范围和优先级，不再重复维护任务状态。
...
| 现有组件完成定义复核 | 待完善 | 逐个补齐 58 个已实现组件的状态示例、组件特有 API/Token 说明、交互测试和对比度证据 |
| 剩余 P1 | 待实现 | 28 个条目，按明确产品场景进入后续里程碑 |
...
其余标记为 P1 但未列入本里程碑核心批次的组件继续保持待实现，按后续明确里程碑推进。
...
5. Gallery 单组件详情页、程序化交互测试、58 份已实现组件规格和软件渲染截图已进入质量门禁。

P2 的筛选、日期历法、格式化、DataGrid 窗口数据、焦点恢复和平台通知均采用显式宿主协议；组件不内建业务数据策略，也不通过静默回退伪装平台能力。具体边界见各组件规格，后续补齐任务见 [`../TODO.md`](../TODO.md)。
