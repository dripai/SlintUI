...
每个组件的状态为“待实现”“实现中”“已完成”之一。当前仓库已完成全部 P0、提前完成的 `ScrollArea`、里程碑 D 核心桌面批次，以及里程碑 E 的全部 20 个 P2 核心组件；其余 P1 与平台增强仍为待实现。逐组件状态、API 和限制见 [`component-specs/`](component-specs/README.md)。
...
5. Gallery 第 7 页、程序化交互测试、58 份已实现组件规格和软件渲染截图已进入质量门禁。

P2 的筛选、日期历法、格式化、DataGrid 窗口数据、焦点恢复和平台通知均采用显式宿主协议；组件不内建业务数据策略，也不通过静默回退伪装平台能力。具体边界见各组件规格和 [`../docs/p2-status.md`](../docs/p2-status.md)。
