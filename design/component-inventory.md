...
当前仓库已完成全部 P0、40 个 P1 和 20 个 P2 组件规格，共 86 份；原 58 个组件的完成定义也已复核。已实现 API 与限制见 [`component-specs/`](component-specs/README.md)，质量证据见 [`component-quality-audit.md`](component-quality-audit.md)；平台验证、产品接入和发布任务统一在 [`../TODO.md`](../TODO.md) 维护。
...
| 里程碑 C 的 P0 验收 | 已完成 | Gallery 代表性设置页和工具栏、主题、密度、缩放、Locale、RTL、交互测试、截图和 `v0.1.0` 固定版本 |
| 里程碑 D 核心桌面批次 | 已完成 | 菜单、Tabs、Table、Tree、Dialog、Toast、SplitPane、Form、Overlay；Gallery、交互测试和截图已补齐 |
| 里程碑 E P2 扩展批次 | 已完成 | 20 个 P2 操作、导航、录入、展示与反馈组件；Gallery、交互测试、规格和截图已补齐 |
| 现有组件完成定义复核 | 已完成 | 58 个原有组件已补齐状态说明、组件特有 API/Token、交互证据、对比度记录和 125%/200% 截图 |
| 剩余 P1 | 已完成 | 28 个条目已实现并进入分类 Gallery、规格和交互门禁；P1 合计 40 份规格 |
| 平台增强 | 待实现 | 6 项系统能力，需独立 crate 和三平台统一接口 |
| 平台无障碍与真实 DPI 验证 | 待实现 | 语义已声明；Narrator、NVDA、VoiceOver、Orca 和三平台真机矩阵尚未执行 |
| 真实业务产品接入 | P0 豁免 | 当前仓库没有可供迁移的外部业务页面；不虚构验证结果，也不以该外部条件阻塞核心库 P0 基线 |
...
| 组件 | 优先级 | 关键能力 |
|---|---:|---|
| `Label` | P0 | 换行、继承 Text 的可配置截断和可访问名称；Tooltip 由调用方组合，P0 不承诺助记键 |
| `Badge` | P1 | 状态点、计数和上限 |
| `Tag` | P1 | 状态、可关闭、可选择 |
...
### 里程碑 C：P0 验收与固定版本（已完成）

1. 使用 Gallery 代表性设置页验证表单、错误、长文本、Locale、RTL 和焦点顺序。
2. 使用 Gallery 代表性工具栏验证密度、快捷键、Tooltip、选中态和禁用行为。
3. 通过交互测试和软件渲染截图验收后发布 `v0.1.0` 固定版本，业务项目不复制组件源码。

外部真实业务项目接入不属于 `v0.1.0` 的阻塞门禁：当前仓库没有可迁移的外部页面，必须在首个采用方出现后单独记录，不能用 Gallery 冒充业务验证。

### 里程碑 D：常用桌面能力（核心批次已完成）

1. 已实现 PopupMenu、PopupMenuItem、ContextMenu、Tabs、Table、Tree、ModalDialog、ConfirmDialog、Toast、SplitPane、Form 和 Overlay。
2. Gallery、交互测试、组件规格和代表性截图已进入质量门禁。
3. 原生窗口拖动、文件对话框、系统托盘、全局快捷键和通知仍属于“平台”优先级，不进入核心 crate，也不以条件分支伪装实现。

其余 P1 组件已在里程碑 F 完成。

### 里程碑 E：P2 扩展能力（已完成）

1. 操作与导航：SplitButton、Breadcrumb、Steps、Pagination、NavigationRail、CommandPalette。
2. 数据录入：MultiSelect、AutoComplete、DatePicker、TimePicker、ColorPicker。
3. 数据展示：Avatar、DataGrid、Statistic、Timeline、ImageView、Calendar。
4. 反馈与浮层：Notification、Drawer、Skeleton。
5. Gallery 单组件详情页、程序化交互测试、当时的 58 份已实现组件规格和软件渲染截图已进入质量门禁。

P2 的筛选、日期历法、格式化、DataGrid 窗口数据、焦点恢复和平台通知均采用显式宿主协议；组件不内建业务数据策略，也不通过静默回退伪装平台能力。具体边界见各组件规格，后续平台与产品任务见 [`../TODO.md`](../TODO.md)。

### 里程碑 F：P1 完整批次与质量复核（已完成）

1. 操作：ButtonGroup、ToggleButton、Link。
2. 布局、导航与桌面核心：Flex、Grid、Panel、AppShell、Dropdown、ShortcutHint、TitleBar。
3. 数据录入：TextArea、SearchField、NumberInput、CheckboxGroup、Radio/RadioGroup、ComboBox、Slider、FilePicker。
4. 数据展示与反馈：Badge、Tag、List/ListItem、DescriptionList、Collapse、Popover、Alert、ProgressRing、ResultState、LoadingOverlay。
5. 28 个组件均有公开 API、分类 Gallery、规格和交互/静态证据；FilePicker 与 TitleBar 只发出宿主/平台请求，不伪装原生文件对话框或窗口管理。
6. 原 58 个组件同步完成状态、API/Token、边界交互、对比度与缩放证据复核；详见 [`component-quality-audit.md`](component-quality-audit.md)。
