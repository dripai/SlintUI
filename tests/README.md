...
自动交互 Harness 覆盖 Button、IconButton、ToolButton、Checkbox、Switch、SegmentedControl、TextField 清除和 Select 程序化选择，并验证禁用、重复与越界路径不会产生额外状态或回调。
...
现有 7 张基线覆盖浅色、深色、高对比度、RTL、compact/regular/comfortable、100%/150% 预览缩放和 outlined/filled 图标目录。125%/200% 可在 Gallery 中交互检查，但不为每种组合保留固定图片。截图使用 `winit-software` 并强制减少动效，使动画相位不会造成无意义差异；跨系统字体差异仍需人工评审。
