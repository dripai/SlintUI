...
当前阶段：公开入口包含 88 个组件和 1 个 `Theme` 全局对象，共 89 份独立规格，全部按 `Alpha` 重新评审。历史 P0/P1/P2 的“已实现”只代表存在源码和演示入口，不代表公开 API、行为或平台验证达到生产级。后续收敛任务见 [`TODO.md`](TODO.md)。
...
公开组件按通用、布局、导航、数据录入、数据展示、反馈和桌面分类。逐项公开 API、视觉规范、行为规范和当前限制见 [`design/component-specs/`](design/component-specs/README.md)，未完成工作见 [`TODO.md`](TODO.md)。
...
│   ├── interaction.md
│   ├── accessibility.md
│   ├── content-and-localization.md
│   ├── gallery-information-architecture.md
│   └── component-specs/<category>/*.md
├── docs/
│   ├── getting-started.md
│   ├── iconography.md
│   └── theming.md
...
Gallery 固定使用“Ant Design 一级分类 → 单个公开组件”的两层导航，不展示 P0、P1、P2 或里程碑。右侧一次只说明一个组件，按“用途与何时使用 → 独立示例 → API 与限制 → Theme Token”的顺序组织；完整约束见 [`design/gallery-information-architecture.md`](design/gallery-information-architecture.md)。
...
- [`design/component-specs/`](design/component-specs/README.md)：定义公开组件范围、分类，以及每个组件的公开 API、视觉规范、行为规范和当前限制。
- [`design/gallery-information-architecture.md`](design/gallery-information-architecture.md)：定义 Gallery 的分类、两层导航和单组件详情页结构。
...
首个固定历史基线为 Git Tag `v0.1.0`；当前公开范围和 Alpha 契约见 [`design/component-specs/`](design/component-specs/README.md)，后续工作见 [`TODO.md`](TODO.md)。
...
### 阶段二：Gallery（骨架已建立，生产级展示继续收敛）

1. 创建独立 Gallery 应用。
2. 实现组件导航、主题切换和缩放预览。
3. 已建立组件状态展示模板，正在按单组件真实 API、视觉和行为规格补齐交互文档。
4. 已建立截图基线目录；分类化命名和完整环境矩阵仍在 [`TODO.md`](TODO.md) 跟踪。

### 阶段三：组件 Alpha 收敛（进行中）

当前源码已覆盖以下首批组件，仍需按生产级契约顺序评审：
...
当前范围和完成标准以 [`design/component-specs/`](design/component-specs/README.md) 与项目级 [`AGENTS.md`](AGENTS.md) 为准。后续不继续追求组件数量，优先保证设计一致、状态完整和接口稳定。

### 阶段四：首个产品验证（待实现）

`v0.1.0` 已经发布；本阶段剩余任务为：

1. 由首个业务项目通过固定 Git tag 接入。
2. 优先迁移设置页，验证表单类组件。
3. 再迁移工具栏，验证高频交互组件。
4. 组件问题回到 SlintUI 修复，禁止在业务项目中复制后修改。
...
- `Select` 复用标准 `ComboBox` 以保留原生弹层、键盘和焦点行为。标准控件样式在编译期选择，不能随 SlintUI 的运行时主题完全重绘；搜索型选择器使用独立 AutoComplete/ComboBox 协议。
...
- `Flex` 当前提供水平伸缩排列，垂直方向继续使用 `Stack`；`Grid` 的 Row/单元格结构在 Slint 编译期确定，两者都不承诺运行时自动换行。
- `NumberInput` 基于原生 `SpinBox`，当前公开整数步进；`FilePicker` 只发出宿主选择请求，不内建或静默替代平台文件对话框。
- `ProgressRing` 的紧凑圆弧按四分之一阶段绘制，精确值仍通过文字和可访问值暴露；`TitleBar` 只提供外观与窗口操作请求，实际拖动、最小化和最大化由平台增强层执行。
- Gallery 的缩放是设计预览环境；Windows、macOS、Linux 的真实 DPI、字体、输入法与屏幕阅读器仍需分别验证。

未完成的平台和产品验证任务见 [`TODO.md`](TODO.md)。
