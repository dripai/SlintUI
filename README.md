...
例如，组件库可以提供 `SlintToolButton`，但不提供 `ScreenshotButton` 或 `StartRecordingButton`。业务项目通过属性和回调组合通用组件。
...
│   │       └── lib.rs          # 统一平台能力接口
│   │
│   ├── slint-ui-platform-windows/
│   │   ├── Cargo.toml
│   │   └── src/
│   │       └── lib.rs
│   │
│   ├── slint-ui-platform-macos/
│   │   ├── Cargo.toml
│   │   └── src/
│   │       └── lib.rs
│   │
│   └── slint-ui-platform-linux/
│       ├── Cargo.toml
│       └── src/
│           └── lib.rs
│
...
export component SlintToolButton {
...
- [`design/foundations.md`](design/foundations.md)：定义颜色、排版、间距、尺寸、圆角、边框、阴影、动效、主题和无障碍基线。
- [`design/component-inventory.md`](design/component-inventory.md)：定义完整组件清单、分层、优先级、目标文件和完成标准。
- `design/component-specs/`：后续为每个组件记录用途、API、状态、键盘行为、可访问语义和视觉验收要求。
